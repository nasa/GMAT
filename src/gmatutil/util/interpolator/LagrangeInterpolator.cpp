...
Integer LagrangeInterpolator::IsInterpolationFeasible(Real ind)
{
   ...
   // Reset order setting if needed before checking feasibility
   if (resetToBaseOrder && (order != baseOrder))
   {
      order = baseOrder;
      requiredPoints = order + 1;
   }
...
//------------------------------------------------------------------------------
//  bool AdjustOrderToNumPoints()
//------------------------------------------------------------------------------
/**
 * Attempts to temporarily reduce the order of the interpolator according to
 * the number of points currently available. Note that after the next call
 * to Interpolate(), the order will automatically be restored to its original
 * value.
 *
 * @return  true  successfully able to reduce the order
 *          false order was not able to be changed 
 */
//------------------------------------------------------------------------------
bool LagrangeInterpolator::AdjustOrderToNumPoints()
{
   if (pointCount > 1 && pointCount < requiredPoints)
   {
      order = pointCount - 1;
      requiredPoints = order + 1;
      resetToBaseOrder = false;
      return true;
   }
   else
      return false;
...
bool LagrangeInterpolator::Interpolate(const Real ind, Real *results)
{
   ...
   Integer feasFlag = IsInterpolationFeasible(ind);
   // We will want to reset the order next time unless otherwise specified
   resetToBaseOrder = true;
   if (feasFlag != 1)
   {
      #ifdef DEBUG_LAGRANGE_INTERPOLATE
      MessageInterface::ShowMessage
...
{
#ifdef DEBUG_LAGRANGE_BUILD
    MessageInterface::ShowMessage("Lagrange::UpdateBeginAndEndIndex() entered, ind=%f, beginIndex=%d, endIndex=%d, actualSize=%d, order=%d\n", ind, beginIndex, endIndex, actualSize, order);
#endif

    if (actualSize < requiredPoints)
    {
#ifdef DEBUG_LAGRANGE_BUILD
        MessageInterface::ShowMessage("Lagrange::UpdateBeginAndEndIndex() false : insufficient points\n");
#endif
        return false;
    }
    ...
    for (Integer i = 1; i < actualSize; i++)
        if (fabs(x[i] - ind) < distToDataIndex)
        {
            dataIndex = i;
            distToDataIndex = fabs(x[i] - ind);
        }

    if (requiredPoints % 2 == 0)
    {
    ...
    #ifdef DEBUG_LAGRANGE_BUILD
    MessageInterface::ShowMessage("   dataIndex=%d, beginIndex=%d, endIndex=%d\n", dataIndex, beginIndex, endIndex);
    MessageInterface::ShowMessage("Lagrange::UpdateBeginAndEndIndex() returning true\n");
    #endif
