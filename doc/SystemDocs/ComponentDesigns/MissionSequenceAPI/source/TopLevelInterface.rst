...
.. todo::  Things needed to support multi-sequence 
	
	Command        	  Command(type, desc, seq)     GmatCommand  Creates a GmatCommand and adds it to the end of the given sequence 

	Execute           Execute(seq)                 Int      	Runs the given sequence of GMATCommands
...
The corresponding API configuration is shown in :numref:`APITargeting`.  The sequence ordering is established using the AddCommand function calls, as shown in the listing.
